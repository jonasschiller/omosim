...
class TrafficSensor(
    ...
    companion object {
        fun readSensorData(linkData: File, omod: Omod) : List<TrafficSensor> {
        ...
        fun affectedSensors(
            ...
            val affectedSensors: Map<Pair<RealLocation, RealLocation>, List<TrafficSensor>> = runBlocking(omod.dispatcher) {
                channelFlow {
                    for (origin in omod.grid) {
                        launch {
                            for (destination in omod.grid) {
                                val response = routeWith("car", origin, destination, omod.hopper!!)
        ...
        fun altAffectedSensors(
            ...
            val affectedSensors: Map<Pair<RealLocation, RealLocation>, List<List<TrafficSensor>>> = runBlocking(omod.dispatcher) {
                channelFlow {
                    for (origin in omod.grid) {
                        launch {
                            for (destination in omod.grid) {
                                val response = routeAltCar(origin, destination, omod.hopper!!)
